<!-- =========================================================================================
    File Name:  Leads List
    Description: These Details are Viewed by Sales
     ========================================================================================== -->
                                ...
                                <div class="flex items-center">
                                    <div class="con-vs-avatar flex-shrink-0 mr-2 30px"
                                        ...
                                        <div class="con-img vs-avatar--con-img">
                                            ...
                                            <img v-show="tr.profileUrl == '' && gender=='male'"
                                                 src="../../../assets/images/portrait/small/default-profile.png"
                                                 style="width: 60px; height: 60px;">
                                            <img v-show="tr.profileUrl == '' && gender=='female'"
                                                 src="../../../assets/images/portrait/small/female_avatar_vector.png"
                                                 style="width: 60px; height: 60px;">
                                            <img v-show="tr.profileUrl == '' && gender=='other'"
                                                 src="../../../assets/images/portrait/small/default-profile.png"
                                                 style="width: 60px; height: 60px;">

                                        </div>
                                    </div>
                                    <!--<a @click="assignLead(tr.id,tr.name )" class="text-inherit hover:text-primary">{{ tr.name }}</a>-->
                                    <a @click="openConfirm()" class="text-inherit hover:text-primary">{{ tr.name }}</a>
                                </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getLeads(){
                firebase.firestore().collection('leads').where('lead_owner', '==', localStorage.getItem('uid')).orderBy('createdAt', 'desc').get().then((querySnapshot) => {
                    this.trainers = []
                    querySnapshot.forEach((doc) => {
                        if (doc.data().lead_status != 'hgWSF4yia2KUmP4oVYq9' && doc.data().isLead != false) {
                            const trainer = {
                                docID: doc.id,
                                id: doc.id,
                                data: doc.data(),
...
//          deadLeads
                firebase.firestore().collection('leads').where('lead_owner', '==', localStorage.getItem('uid')).where('lead_status', '==', 'hgWSF4yia2KUmP4oVYq9').orderBy('createdAt', 'desc').get().then((querySnapshot) => {
                    this.deadLeads = []
                    querySnapshot.forEach((doc) => {
                        const trainer = {
                            docID: doc.id,
...
//                    this.assignLeads=[]
                    querySnapshot.forEach((doc) => {
                        firebase.firestore().collection('leads').doc(doc.data().lead_id).get().then((docs) => {
                            if (docs.data().lead_status != 'hgWSF4yia2KUmP4oVYq9' && docs.data().isLead != false) {
                                const assign = {
                                    docID: docs.id,
                                    id: docs.id,
                                    data: docs.data(),
