<!-- =========================================================================================
    File Name:  Leads Source
    Description: These Details are added by Admin, this page is for add edit and delete lead source
     ========================================================================================== -->

<template>
    <vx-card>
        <vs-table pagination max-items="25" search :data="status" stripe>
                ...
                <div class="vx-row">
                    ...
                    <div class="vx-col">
                        <button class="button-save p-3 mb-10 mr-4 rounded-lg vs-align-center cursor-pointer text-lg font-medium text-base text-primary border border-solid border-primary save"
                                v-if="!addNewFlag" @click="saveStatus()">
                            <span class="ml-2 text-base text-primary">Save</span>
                        </button>
                        <button class="button-save p-3 mb-10 mr-4 rounded-lg vs-align-center cursor-pointer text-lg font-medium text-base text-primary border border-solid border-primary save"
                                v-if="addNewFlag" @click="updateStatus()">
                ...
                <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
                    ...
                    <vs-td>
                        <div class="p-2 mb-2  rounded-lg cursor-pointer flex items-center justify-between text-sm font-medium text-base text-primary  button edit"
                             @click="editStatus(tr.uid),addNewFlag=true, statusUID = tr.uid">
                        ...
                        <span><a
                                ...
                                button @click="deleteStatus(tr.uid,data[indextr].data.title)"><feather-icon
                                icon="TrashIcon" svgClasses="h-4 w-4"/></a> </span>
                    </vs-td>
                </vs-tr>
            </template>
        </vs-table>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getStatus(){
                firebase.firestore().collection('lead_status').orderBy('createdAt', 'desc').get().then((querySnapshot) => {
                    this.status = []
            ...
            saveStatus(){
                this.$validator.validateAll().then(result => {
                    if (result) {
                        ...
                        firebase.firestore().collection("lead_status").add(
                            memberPayLoad
                        )
                            .then(() => {
                                    this.getStatus();
            ...
            deleteStatus(uid, title) {
                let title_text = title;
                firebase.firestore().collection('lead_status')
                    .doc(uid).delete()
                    .then(() => {
                        this.getStatus()
            ...
            editStatus(uid){
                firebase.firestore().collection('lead_status').doc(uid).get().then((doc) => {
                    let value = doc.data();
            ...
            updateStatus(){
                this.$validator.validateAll().then(result => {
                    if (result) {
                        this.loadContent();
                        firebase.firestore().collection('lead_status').doc(this.statusUID).get()
                            .then(snap => {
