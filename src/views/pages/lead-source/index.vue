...
<template>
    <vx-card>
        <vs-table pagination max-items="25" search :data="source" stripe>
                ...
                <div class="vx-row">
                    ...
                    <div class="vx-col">
                        <button class="button-save p-3 mb-10 mr-4 rounded-lg vs-align-center cursor-pointer text-lg font-medium text-base text-primary border border-solid border-primary save"
                                v-if="!addNewFlag" @click="saveSource()">
                            <span class="ml-2 text-base text-primary">Save</span>
                        </button>
                        <button class="button-save p-3 mb-10 mr-4 rounded-lg vs-align-center cursor-pointer text-lg font-medium text-base text-primary border border-solid border-primary save"
                                v-if="addNewFlag" @click="updateSource()">
                ...
                <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
                    ...
                    <vs-td>
                        <div class="p-2 mb-2 rounded-lg cursor-pointer flex items-center justify-between text-sm font-medium text-base text-primary  edit"
                             @click="editSource(tr.uid),addNewFlag=true, sourceUID = tr.uid">
                        ...
                        <span><a
                                ...
                                button @click="deleteSource(tr.uid,tr.data.title)"><feather-icon icon="TrashIcon"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getSource(){
                firebase.firestore().collection('lead_source').orderBy('createdAt', 'desc').get().then((querySnapshot) => {
                    this.source = []
            ...
            saveSource(){
                this.$validator.validateAll().then(result => {
                    if (result) {
                        ...
                        firebase.firestore().collection("lead_source").add(
                            memberPayLoad
                        )
                            .then(() => {
                                    this.getSource();
            ...
            deleteSource(uid, title) {
                let title_text = title;
                firebase.firestore().collection('lead_source')
                    .doc(uid).delete()
                    .then(() => {
                        this.getSource()
            ...
            editSource(uid){
                firebase.firestore().collection('lead_source').doc(uid).get().then((doc) => {
                    let value = doc.data();
            ...
            updateSource(){
                this.$validator.validateAll().then(result => {
                    if (result) {
                        this.loadContent()
                        firebase.firestore().collection('lead_source').doc(this.sourceUID).get()
                            .then(snap => {
